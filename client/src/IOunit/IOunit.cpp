...
bool IOunit::addIStream(std::shared_ptr<std::istream> Istream) {
    if (!Istream->good() ||
        std::find(INstreams.begin(), INstreams.end(), Istream) != INstreams.end()) {
        return false;
        // Идея - сделать более понятные значение возврата (возвращать исключения будет так себе,
        // это относитльно ожидаемые ситуации)
    }
...
bool IOunit::addOStream(std::shared_ptr<std::ostream> Ostream) {
    if (!Ostream->good() ||
        std::find(OUTstreams.begin(), OUTstreams.end(), Ostream) != OUTstreams.end()) {
        return false;
        // Идея - сделать более понятные значение возврата (возвращать исключения будет так себе,
        // это относитльно ожидаемые ситуации)
    }
