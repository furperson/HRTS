...
int main(int argc, char* argv[]) {
    ...
    // crypto_pwhash_str(hashed_password, str.c_str(), str.length(),
    //                   crypto_pwhash_OPSLIMIT_MODERATE, crypto_pwhash_MEMLIMIT_MODERATE);
    // std::cout << hashed_password << std::endl;
    if (argc <6){
        std::cout << "введено мало аргументов, небходимо :  \n порт_сервер pg_host pg_port pg_table pg_user \n";
        exit(1);
    }
    ...
    // pinNaming1.emplace_back("DATA_BUS_HIGH", std::vector<int>{8, 9, 10, 11, 12, 13, 14, 15},
    // true); pinNaming1.emplace_back("ADDR_BUS", std::vector<int>{16, 17, 18, 19, 20}, false);
    ...
    server.init(static_cast<int>(atoi(argv[1])));
